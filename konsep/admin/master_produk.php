...
<div class="flex items-center justify-between mb-5 flex-wrap gap-3">
    <div>
        <h2 class="text-base font-bold text-gray-900">Daftar Produk Induk</h2>
        <p class="text-xs text-gray-500 mt-0.5">Total: <?= count($produkInduk) ?> produk &bull; Klik &blacktriangledown; untuk lihat variasi & tambah variasi</p>
    </div>
...
<?php foreach ($produkInduk as $pi): ?>
    ...
    ?>
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden <?= !$aktif ? 'opacity-60' : '' ?>">
        <!-- Header row -->
        <div class="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-slate-50 transition"
            ...
            <div class="flex items-center gap-3 min-w-0">
                <span class="text-gray-500"><?= icon('pill') ?></span>
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <span class="text-sm font-bold text-gray-900"><?= htmlspecialchars($pi['nama_produk']) ?></span>
                        <span class="px-2 py-0.5 bg-slate-100 border border-gray-200 rounded-lg text-[10px] font-semibold"><?= htmlspecialchars($pi['kategori']) ?></span>
        ...
        <div id="acc_<?= $pi['id'] ?>" class="hidden border-t border-gray-200 bg-slate-50">
            <div class="p-4">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-xs font-bold text-gray-900">Daftar Variasi</span>
                    <button onclick="openTambahVariasi(<?= $pi['id'] ?>, '<?= addslashes($pi['nama_produk']) ?>')"
                        class="text-[11px] font-bold px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded-xl transition">+ Tambah Variasi</button>
                </div>
                <?php if (empty($variasiList)): ?>
                    <p class="text-xs text-gray-500 italic py-4 text-center">Belum ada variasi. Tambahkan variasi untuk produk ini.</p>
                <?php else: ?>
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs">
                            <thead class="text-gray-500 uppercase tracking-wider font-bold border-b border-gray-200">
                                <tr>
                                ...
                                <?php foreach ($variasiList as $v): ?>
                                    <tr class="<?= !$v['is_active'] ? 'opacity-50' : '' ?>">
                                        <td class="py-2.5 px-3">
                                            <div class="font-mono text-gray-500 text-[10px]"><?= htmlspecialchars($v['sku_variasi']) ?></div>
                                            <div class="font-semibold text-gray-900 text-xs"><?= htmlspecialchars($v['nama_variasi']) ?></div>
                                        </td>
                                        <td class="py-2.5 px-3 text-center">
                                            <div class="text-[10px] text-gray-500">1 <?= htmlspecialchars($v['satuan_besar']) ?> = <?= $v['rasio_konversi'] ?> <?= htmlspecialchars($v['satuan_kecil']) ?></div>
                                        </td>
                                        <td class="py-2.5 px-3 text-right font-bold text-slate-700">Rp <?= number_format($v['harga_jual_kecil'], 0, ',', '.') ?> <span class="text-[9px] font-normal text-gray-500">/ <?= htmlspecialchars($v['satuan_kecil']) ?></span></td>
...
<div id="modal_tambah_induk" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-sm font-bold text-gray-900">Tambah Produk Induk Baru</h3>
            <button onclick="document.getElementById('modal_tambah_induk').classList.add('hidden')" class="text-gray-500 hover:text-gray-900 text-lg">&times;</button>
        </div>
        <form method="POST" class="p-6 space-y-4">
            <input type="hidden" name="aksi" value="tambah_induk">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">SKU Induk *</label>
                    <input type="text" name="sku_induk" required placeholder="OBT-2026-0004-IND" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Kategori *</label>
                    <select name="kategori" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Nama Produk *</label>
                <input type="text" name="nama_produk" required placeholder="Contoh: Masker N95 3M" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Deskripsi</label>
                <textarea name="deskripsi" rows="2" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50" placeholder="Keterangan singkat produk..."></textarea>
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Supplier</label>
                <select name="id_supplier" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" class="px-5 py-2 text-xs font-bold bg-gray-800 hover:bg-gray-700 text-white rounded-xl transition shadow-sm">Simpan Produk</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal_edit_induk" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-sm font-bold text-gray-900">Edit Produk Induk</h3>
            <button onclick="document.getElementById('modal_edit_induk').classList.add('hidden')" class="text-gray-500 hover:text-gray-900 text-lg">&times;</button>
        </div>
        <form method="POST" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Nama Produk *</label>
                <input type="text" name="nama_produk" id="edit_nama_produk" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Kategori *</label>
                    <select name="kategori" id="edit_kategori" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                        <option value="Obat-obatan">Obat-obatan</option>
                        <option value="Alat Monitor">Alat Monitor</option>
                        <option value="Alat Bantu Jalan">Alat Bantu Jalan</option>
                        <option value="Perawatan Luka">Perawatan Luka</option>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Supplier</label>
                    <select name="id_supplier" id="edit_supplier" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Deskripsi</label>
                <textarea name="deskripsi" id="edit_deskripsi" rows="2" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50"></textarea>
            </div>
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" class="px-5 py-2 text-xs font-bold bg-gray-800 hover:bg-gray-700 text-white rounded-xl transition shadow-sm">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal_tambah_variasi" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-sm font-bold text-gray-900">Tambah Variasi - <span id="var_parent_name" class="text-gray-950"></span></h3>
            <button onclick="document.getElementById('modal_tambah_variasi').classList.add('hidden')" class="text-gray-500 hover:text-gray-900 text-lg">&times;</button>
        </div>
        <form method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">SKU Variasi *</label>
                    <input type="text" name="sku_variasi" required placeholder="OBT-2026-0004-100ML" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Nama Variasi *</label>
                    <input type="text" name="nama_variasi" required placeholder="100ml / Merah / etc" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>
            
            <div class="grid grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Satuan Kecil (Ecer) *</label>
                    <input type="text" name="satuan_kecil" required placeholder="Pcs/Strip" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Satuan Besar (Grosir) *</label>
                    <input type="text" name="satuan_besar" required placeholder="Box/Karton" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Rasio (1 Besar = ? Kecil)</label>
                    <input type="number" name="rasio_konversi" value="1" min="1" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>
            
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Harga Jual Kecil (Ecer) *</label>
                    <input type="number" name="harga_jual_kecil" required min="0" step="500" placeholder="5000" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Harga Jual Besar (Grosir) *</label>
                    <input type="number" name="harga_jual_besar" required min="0" step="500" placeholder="450000" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Berat (gram) *</label>
                    <input type="number" name="berat" required min="1" placeholder="250" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Tampilkan di E-Commerce?</label>
                    <label class="inline-flex items-center mt-2.5">
                        <input type="checkbox" name="tampil_di_online" value="1" checked class="rounded border-slate-300 text-gray-800 focus:ring-gray-500 h-4 w-4">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">URL Gambar Produk</label>
                <input type="text" name="gambar" placeholder="https://..." class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            </div>
...
<div id="modal_edit_variasi" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-sm font-bold text-gray-900">Edit Variasi</h3>
            <button onclick="document.getElementById('modal_edit_variasi').classList.add('hidden')" class="text-gray-500 hover:text-gray-900 text-lg">&times;</button>
        </div>
        <form method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">SKU Variasi *</label>
                    <input type="text" name="sku_variasi" id="edit_sku_variasi" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Nama Variasi *</label>
                    <input type="text" name="nama_variasi" id="edit_nama_variasi" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Satuan Kecil (Ecer) *</label>
                    <input type="text" name="satuan_kecil" id="edit_satuan_kecil" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Satuan Besar (Grosir) *</label>
                    <input type="text" name="satuan_besar" id="edit_satuan_besar" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Rasio (1 Besar = ? Kecil)</label>
                    <input type="number" name="rasio_konversi" id="edit_rasio_konversi" min="1" required class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Harga Jual Kecil (Ecer) *</label>
                    <input type="number" name="harga_jual_kecil" id="edit_harga_jual_kecil" required min="0" step="500" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Harga Jual Besar (Grosir) *</label>
                    <input type="number" name="harga_jual_besar" id="edit_harga_jual_besar" required min="0" step="500" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Berat (gram) *</label>
                    <input type="number" name="berat" id="edit_berat_variasi" required min="1" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-900 mb-1.5">Tampilkan di E-Commerce?</label>
                    <label class="inline-flex items-center mt-2.5">
                        <input type="checkbox" name="tampil_di_online" id="edit_tampil_di_online" value="1" class="rounded border-slate-300 text-gray-800 focus:ring-gray-500 h-4 w-4">
                        <span class="ml-2 text-xs font-medium text-slate-700">Tampilkan Online</span>
                    </label>
                </div>
            </div>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">URL Gambar Produk</label>
                <input type="text" name="gambar" id="edit_gambar_variasi" class="w-full text-xs border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-gray-400 bg-slate-50">
            </div>
...
<div id="modal_import_csv" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm flex items-center justify-center">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Pilih File (.csv) *</label>
                <input type="file" name="file_csv" accept=".csv" required class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 bg-slate-50 focus:outline-none focus:border-gray-400">
            </div>
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" onclick="document.getElementById('modal_import_csv').classList.add('hidden')" class="px-5 py-2 text-xs font-bold text-gray-500 hover:bg-slate-100 rounded-xl transition">Batal</button>
