...
<div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
    ...
    <div class="lg:col-span-2 bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
        ...
        <form method="POST" class="space-y-4">
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Produk / Variasi *</label>
                <select name="id_variasi" required class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 bg-white focus:outline-none focus:border-gray-400" onchange="updateSatuan(this)">
            ...
            <div id="produk_info" class="hidden bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs space-y-1">
                <div class="flex justify-between"><span class="text-gray-500">SKU:</span><strong id="info_sku">-</strong></div>
                <div class="flex justify-between"><span class="text-gray-500">Satuan Kecil:</span><strong id="info_kecil">-</strong></div>
                <div class="flex justify-between"><span class="text-gray-500">Satuan Besar:</span><strong id="info_besar">-</strong></div>
                <div class="flex justify-between"><span class="text-gray-500">Rasio Konversi:</span><strong id="info_rasio">-</strong></div>
            ...
            <?php if ($isAdmin): ?>
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Cabang Tujuan *</label>
                <select name="id_cabang" required class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 bg-white focus:outline-none focus:border-gray-400">
            ...
            <div>
                ...
                <div class="text-xs border border-gray-200 rounded-xl px-3 py-2 bg-slate-50 text-gray-500">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Jenis Mutasi *</label>
                <select name="jenis" required class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 bg-white focus:outline-none focus:border-gray-400">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Jumlah (Satuan Terkecil / Pcs) *</label>
                <input type="number" name="qty" min="1" required placeholder="Contoh: 100 (untuk 1 Box isi 100)" class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:border-gray-400">
                <p class="text-[10px] text-gray-500 mt-1">Input selalu dalam satuan terkecil (Pcs/Strip/Unit). Sistem menyimpan stok dalam satuan terkecil.</p>
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-900 mb-1.5">Keterangan (Opsional)</label>
                <input type="text" name="keterangan" placeholder="Contoh: PO-001 dari PT. Kimia Farma" class="w-full text-xs border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:border-gray-400">
            </div>
    ...
    <div class="lg:col-span-3 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center gap-2">
            ...
            <h2 class="text-sm font-bold text-gray-900">Log Penerimaan Stok Terbaru</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-zcBrd/60">
                    ...
                    <tr><td colspan="5" class="px-4 py-10 text-center text-gray-500 italic">Belum ada data penerimaan stok.</td></tr>
                    <?php else: ?>
                    <?php foreach ($recentLogs as $log): ?>
                    <tr class="hover:bg-slate-50/60 transition">
                        <td class="px-4 py-3 font-mono text-gray-500 text-[10px]"><?= date('d/m/y H:i', strtotime($log['tanggal'])) ?></td>
                        <td class="px-4 py-3">
                            <span class="font-semibold text-gray-900 block leading-snug"><?= htmlspecialchars($log['nama_item']) ?></span>
                            <span class="text-[10px] text-gray-500"><?= htmlspecialchars($log['keterangan'] ?? '') ?></span>
                        </td>
                        <td class="px-4 py-3 text-gray-500"><?= htmlspecialchars($log['nama_cabang']) ?></td>
                        <td class="px-4 py-3 text-right font-bold text-emerald-600">+<?= number_format($log['qty']) ?></td>
                        <td class="px-4 py-3 text-right font-bold text-gray-900"><?= number_format($log['sisa_stok']) ?></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
